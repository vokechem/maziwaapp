  ...
  con.getConnection(function (err, connection) {
    if (err) {
      ...
      connection.query(sp, ID, function (error, results, fields) {
        ...
        connection.release();
        //dont use this connection here , it has been returned to the pool
      });
    }
  });
});
  ...
  if (!result.error) {
    ...
    con.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(so, data, function (error, results, fields) {
          if (error) {
            res.json({
              success: false,
              message: "User group successfully Saved",
            });
          }
          connection.release();
          // Don't use the connection here, it has been returned to the pool.
...
usergroups.put("/:ID", auth.validateRole("Security Groups"), function (
  ...
  if (!result.error) {
    const ID = req.params.ID;
    let data = [req.body.Name, req.body.Description, ID, res.locals.user];
    con.getConnection(function (err, connection) {
      if (err) {
        res.json({
          success: false,
